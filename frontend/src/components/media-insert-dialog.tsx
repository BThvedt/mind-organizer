...
interface MediaInsertDialogProps {
  open: boolean;
  onClose: () => void;
  onSelect: (asset: InsertableAsset) => void;
  /** Initial pill selection. Defaults to 'image'. */
  initialType?: InsertableMediaType;
}
...
const TYPE_PILLS: Array<{ id: InsertableMediaType; label: string }> = [
...
export function MediaInsertDialog({
  ...
  const [activeType, setActiveType] = useState<InsertableMediaType>(initialType);
